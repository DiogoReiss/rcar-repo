...
export class WashController {
  ...
  findAll(
    ...
    return this.washService.findAll(inc === 'true', { page: page ?? 1, perPage: perPage ?? 20 });
  }

  @Get(':id')
  @ApiOperation({ summary: 'Detalhe do serviço' })
  findOne(@Param('id', ParseUUIDPipe) id: string) { return this.washService.findOne(id); }
  ...
  update(@Param('id', ParseUUIDPipe) id: string, @Body() dto: UpdateWashServiceDto) { return this.washService.update(id, dto); }

  @Delete(':id')
  @Roles('GESTOR_GERAL')
  @HttpCode(HttpStatus.NO_CONTENT)
  @ApiOperation({ summary: 'Remove (soft-delete) serviço' })
  async remove(@Param('id', ParseUUIDPipe) id: string) { await this.washService.remove(id); }
}
