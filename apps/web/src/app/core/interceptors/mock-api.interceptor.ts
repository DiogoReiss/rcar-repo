...
let mockTemplates: MockTemplate[] = [
  { id: 't1', nome: 'Contrato de Locação', tipo: 'CONTRATO_LOCACAO' as const, ativo: true, createdAt: '2025-01-01T00:00:00Z', variaveis: ['empresaNome', 'dataAtual', 'horaAtual', 'nomeCliente', 'cpfCnpj', 'veiculo', 'placa', 'dataRetirada', 'dataDevolucao', 'valorTotal'], conteudoHtml: '<section style="font-family:Arial,sans-serif"><header style="display:flex;justify-content:space-between;border-bottom:2px solid #0d9488;padding-bottom:8px;margin-bottom:12px"><div><h1 style="margin:0">{{empresaNome}}</h1><small>Data: {{dataAtual}} • Hora: {{horaAtual}}</small></div></header><h2>Contrato de Locação</h2><p><strong>Locatário:</strong> {{nomeCliente}} — {{cpfCnpj}}</p><p><strong>Veículo:</strong> {{veiculo}} — {{placa}}</p><p><strong>Período:</strong> {{dataRetirada}} a {{dataDevolucao}}</p><p><strong>Total:</strong> R$ {{valorTotal}}</p></section>' },
  { id: 't2', nome: 'Recibo de Lavagem',   tipo: 'RECIBO_LAVAGEM'   as const, ativo: true, createdAt: '2025-01-01T00:00:00Z', variaveis: ['empresaNome', 'dataAtual', 'horaAtual', 'nomeCliente', 'servico', 'placa', 'valor', 'data'], conteudoHtml: '<section style="font-family:Arial,sans-serif"><header style="display:flex;justify-content:space-between;border-bottom:2px solid #0d9488;padding-bottom:8px;margin-bottom:12px"><div><h1 style="margin:0">{{empresaNome}}</h1><small>Data: {{dataAtual}} • Hora: {{horaAtual}}</small></div></header><h2>Recibo de Lavagem</h2><p><strong>Cliente:</strong> {{nomeCliente}}</p><p><strong>Serviço:</strong> {{servico}}</p><p><strong>Placa:</strong> {{placa}}</p><p><strong>Data:</strong> {{data}}</p><p style="font-size:18px"><strong>Total: R$ {{valor}}</strong></p></section>' },
  { id: 't3', nome: 'Termo de Vistoria',   tipo: 'VISTORIA'         as const, ativo: true, createdAt: '2025-06-01T00:00:00Z', variaveis: ['empresaNome', 'dataAtual', 'horaAtual', 'nomeCliente', 'veiculo', 'placa', 'km', 'data', 'tipo'], conteudoHtml: '<section style="font-family:Arial,sans-serif"><header style="display:flex;justify-content:space-between;border-bottom:2px solid #0d9488;padding-bottom:8px;margin-bottom:12px"><div><h1 style="margin:0">{{empresaNome}}</h1><small>Data: {{dataAtual}} • Hora: {{horaAtual}}</small></div></header><h2>Termo de Vistoria — {{tipo}}</h2><p><strong>Data:</strong> {{data}} | <strong>Veículo:</strong> {{veiculo}} {{placa}} | <strong>KM:</strong> {{km}}</p><p><strong>Cliente:</strong> {{nomeCliente}}</p></section>' },
  { id: 't4', nome: 'Termo de Responsabilidade', tipo: 'TERMO_RESPONSABILIDADE' as const, ativo: true, createdAt: '2025-06-01T00:00:00Z', variaveis: ['empresaNome', 'dataAtual', 'horaAtual', 'nomeCliente', 'cpfCnpj', 'veiculo', 'placa', 'dataRetirada', 'dataDevolucao', 'valorTotal'], conteudoHtml: '<section style="font-family:Arial,sans-serif"><header style="display:flex;justify-content:space-between;border-bottom:2px solid #0d9488;padding-bottom:8px;margin-bottom:12px"><div><h1 style="margin:0">{{empresaNome}}</h1><small>Data: {{dataAtual}} • Hora: {{horaAtual}}</small></div></header><h2>Termo de Responsabilidade</h2><p>{{nomeCliente}} ({{cpfCnpj}}) declara ciência das condições de locação.</p><p>Veículo: {{veiculo}} - {{placa}}</p><p>Período: {{dataRetirada}} a {{dataDevolucao}}</p><p>Valor previsto: R$ {{valorTotal}}</p></section>' },
];
