...
| 🟡 | 24.9 | Tabela de rentabilidade por veículo: placa, receita acumulada, manutenção acumulada, lucro bruto *(parcial: página atual mostra custo de manutenção por veículo; falta compor receita por veículo para lucro bruto)* |
...
| 🔴 | 24.13 | Integrar gráfico de distribuição por método com endpoint financeiro dedicado (dependência backend `GET /payments` com agregação) |
...
| 🟢 | 23.10 | **Context menu nas tabelas**: coluna Ações das tabelas admin e contrato-list usa `RowMenuComponent` (`lync-row-menu`) em vez de botões inline. Componente reescrito sem PrimeNG, dividido em 3 peças (`row-menu.ts` trigger + `row-menu-panel.ts` overlay + `row-menu-item.ts` item). Tipo próprio `RowMenuItem` (substitui `MenuItem` do primeng/api). Posicionamento `position:fixed` calculado no toggle. Fechamento por clique fora via `(document:click)` no host e por Escape via `(document:keydown.escape)` no panel |
...
| 🟢 | 23.14 | **Histórico de cliente como dialog**: "Ver Histórico" no context menu de `clientes-list` abre `p-dialog` inline com os painéis Dados, Lavagens e Contratos; remove navegação para `/admin/clientes/:id`; footer com Fechar + Editar (`lync-btn`); botão Buscar migrado para `lync-btn` |
| 🟢 | 23.15 | **WizardDialogComponent** (`lync-wizard-dialog`): componente compartilhado de dialog multi-steps com indicador de progresso visual, navegação Voltar/Próximo/Confirmar, e guarda `canNext` controlada pelo parent via input. Usado em Nova Reserva. |
| 🟢 | 23.16 | **Reservas convertidas para wizard**: página `/aluguel/reserva` exibe lista de contratos recentes + botão "Nova Reserva" que abre `lync-wizard-dialog` de 3 passos (Período → Veículo → Detalhes). |
| 🟢 | 23.17 | **Manutenção de frota no context menu**: ações "Registrar Manutenção" (todos veículos) e "Concluir Manutenção" (somente MANUTENCAO) no `row-menu` de `frota-list`. Dialog inline com descrição, custo, data e checkbox "Colocar em manutenção". |
